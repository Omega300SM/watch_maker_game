// ─── COLOUR HELPERS ─────────────────────────────────────────────────────────
import { resolveDialSpec } from '../data/dialSpecs.js'
import { lighten, darken, hexToRgb, rgbToHex } from './colour.js'
import { drawHandShape, drawSecondsShape } from './handsets.js'
import { drawSubdials } from './subdials.js'
import { applyFinish } from './finishes.js'
import { drawMarkers, drawNumerals, drawLogoBlock, drawScale, drawAperture, makeOccupancy, reserve } from './dialFurniture.js'
import { drawBezelVariant } from './bezels.js'
...
export function drawWatch({ canvas, brand, model, parts = [], showHands = false, h = 10, m = 10, s = 30 }) {
  ...
  // Spec-backed colours: default spec reproduces legacy values exactly (parity).
  const cc = caseColor(model)
  const dc = spec.finish === 'skeleton' ? dialColor(model)
           : (spec.baseColor || dialColor(model))
  const bc = spec.bezelSpec?.insert || bezelColor(model)
  const ic = indexColor(model)
  ...
  // Bezel
  if (has('bezel')) {
    drawBezelVariant(ctx, spec, cx, cy, r, bc, ic)
  }
  ...
  if (has('dial')) {
    if (model?.dial==='Skeleton') {
      ...
      applyFinish(ctx, spec, cx, cy, r*0.82, dc)
    }

    // Hour indices / furniture
    if (spec.bespoke) {
      // ── Phase C bespoke path: layout-aware furniture with collision ──
      const occ = makeOccupancy()
      reserve(occ, cx, cy, r * 0.10) // centre pinion / hands hub
      const apertureZone = drawAperture(ctx, spec, cx, cy, r, cc)
      if (apertureZone) reserve(occ, apertureZone.x, apertureZone.y, apertureZone.r)
      const subZones = drawSubdials(ctx, spec, cx, cy, r, dc, ic)
      for (const z of subZones) reserve(occ, z.x, z.y, z.r)
      drawScale(ctx, spec, cx, cy, r, ic)
      drawMarkers(ctx, spec, cx, cy, r, ic, occ)
      drawNumerals(ctx, spec, cx, cy, r, ic, occ)
      if (brand) drawLogoBlock(ctx, spec, brand, model, cx, cy, r, ic, occ)
    } else {
      // ── Legacy-exact path (parity for unauthored models) ──
      for(let i=0;i<12;i++){
      ...
      drawSubdials(ctx, spec, cx, cy, r, dc, ic)
      ...
      if(brand){
        ...
        ctx.fillText(brandText,cx,cy-r*0.28)
        const subline = (logo.lines && logo.lines.length) ? logo.lines[0] : (model?.type||'').toUpperCase()
        ctx.globalAlpha=0.6; ctx.font=`${r*0.045}px 'Josefin Sans',sans-serif`
        ctx.fillText(subline,cx,cy-r*0.18); ctx.globalAlpha=1
      }

      if(spec.complicationsLayout?.aperture === 'date' || spec.complicationsLayout?.aperture === 'day-date'){
  ...
  if(has('hands') && showHands){
    ...
    const hs = spec.handset || 'dauphine'
    // hour
    ...
    drawHandShape(ctx, hs, r*0.45, r*0.04, ic, { lume: spec.markers?.lume !== false }); ctx.restore()
    // minute
    ctx.save(); ctx.translate(cx,cy); ctx.rotate(mA)
    ctx.shadowColor='rgba(0,0,0,0.4)'; ctx.shadowBlur=4; ctx.shadowOffsetX=1; ctx.shadowOffsetY=1
    drawHandShape(ctx, hs, r*0.62, r*0.028, ic, { lume: spec.markers?.lume !== false }); ctx.restore()
    // seconds
    ctx.save(); ctx.translate(cx,cy); ctx.rotate(sA)
    ctx.shadowColor='rgba(255,0,0,0.3)'; ctx.shadowBlur=3
    drawSecondsShape(ctx, spec.secondsHand || 'lollipop', r*0.72); ctx.restore()
